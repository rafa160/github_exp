...
class UserDetailsRepositories extends StatefulWidget {
  ...
  const UserDetailsRepositories({Key key, this.reposUrl}) : super(key: key);
                  ...
                  return ListView.builder(
                      ...
                      itemBuilder: (BuildContext context, int i) {
                        ...
                        return RepositoryCard(
                          repositoryName: item.name != null
                              ? '${item.name}'
                              : Strings.REPOSITORIES_CARD_NAME_EMPTY,
                          repositoryDescription: item.description != null
                              ? '${item.description}'
                              : Strings.REPOSITORIES_DESCRIPTION_TEXT_EMPTY,
                          stars: item.stargazersCount != null
                              ? '${item.stargazersCount.toString()}'
                              : Strings.REPOSITORIES_STARS_CARD_EMPTY,
                          userName: item.user.login != null
                              ? '${item.user.login}'
                              : Strings.REPOSITORIES_USER_CARD_EMPTY,
                          date: item.language != null
                              ? '${item.language}'
                              : Strings.REPOSITORIES_LANGUAGE_CARD_EMPTY,
                        );
                      });
                },
              ),
